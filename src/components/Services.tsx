    ...
    <motion.div
      ...
      className={`group relative flex flex-col p-8 md:p-10 rounded-3xl border transition-all duration-300 overflow-hidden ${
        service.featured
          ...
          : "bg-white dark:bg-slate-900 border-gray-100 dark:border-slate-800 text-gray-900 dark:text-white shadow-xl shadow-gray-200/50 hover:shadow-2xl hover:shadow-primary/10"
      }`}
    >
      <motion.div
      ...
      <div
        className={`w-20 h-20 rounded-2xl flex items-center justify-center mb-8 shadow-sm transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6 ${
          service.featured ? "bg-white/10 backdrop-blur-md border border-white/20" : "bg-blue-50 dark:bg-slate-800 border border-blue-100 dark:border-slate-700"
        }`}
      >
        {service.icon}
      </div>
      ...
      <div className="mb-6 flex items-baseline">
        ...
        <span className={`ml-2 text-lg font-medium ${service.featured ? "text-blue-100" : "text-gray-500 dark:text-slate-400"}`}>{service.priceSuffix}</span>
      </div>
      
      <p className={`mb-10 text-lg leading-relaxed ${service.featured ? "text-blue-50/80" : "text-gray-600 dark:text-slate-400"}`}>
        {service.description}
      </p>
      ...
      <div className="flex-grow space-y-5 mb-10">
        {service.features.map((feature: string, fIndex: number) => (
          <div key={fIndex} className="flex items-start gap-4">
            ...
            <span className={`font-medium ${service.featured ? "text-white" : "text-gray-700 dark:text-slate-300"}`}>
              {feature}
            </span>
          </div>
        ))}
      ...
      <Link
        ...
        className={`w-full py-4 px-6 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2 group/btn ${
          service.featured
            ...
            : "bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-white border border-gray-200 dark:border-slate-700 hover:bg-gray-100 dark:hover:bg-slate-700 hover:border-gray-300 dark:hover:border-slate-600"
        }`}
      >
        Empezar ahora
...
export default function Services() {
  return (
    <section id="servicios" className="py-32 bg-[#fafcff] dark:bg-slate-900 border-y border-gray-100 dark:border-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          <motion.p
            ...
            className="text-xl text-gray-600 dark:text-slate-400 leading-relaxed font-medium"
          >
